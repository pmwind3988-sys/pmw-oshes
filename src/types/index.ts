 ...
 * `field` reads a date out of the submission itself — "expire three days
 * after Permit End Date" — so a permit signed today and one signed next year
 * do not share a deadline. `offsetDays` is the grace after that date; 0 is
 * the day itself, and the link dies at the close of that day in Malaysia.
 *
 * An answer that cannot be read as a date means the link does not expire.
 * That is deliberate, and safe only because a link opens one submission and
 * no other — see `api/_utils/layerItemAccess.ts`.
 */
...
export interface BaseLayer {
   ...
   *
   * Absent means the fixed `tokenExpiresAt` above. Field mode instead names
   * one of the form's own questions, so each submission's link expires on a
   * date its own answers carry. Authored in the pmw-hrform builder and
   * enforced here — see `api/_utils/layerExpiry.ts`.
   */
